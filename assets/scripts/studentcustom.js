...
function selectsubject(){
    ...
    let subject = `<option value="" disabled selected>No Value</option>`;
...
function checkroomvacancy(sem, sy, room) {
    ...
    $('#timeVacancyTitle').html(`Time Vacancy for <strong style="color: red;">${$(roomname).html()}</strong> on <strong style="color: red;">Trimester - ${sem}</strong> S.Y <strong style="color: red;">${sy}</strong>`);
...
function checkcoursevacancy(sem, sy, course) {
    ...
    $('#timeVacancyTitle').html(`Time Vacancy for <strong style="color: red;">${course}</strong> on <strong style="color: red;">Trimester - ${sem}</strong> S.Y <strong style="color: red;">${sy}</strong>`);
...
function checkassignedvacancy(sem, sy, room, teacher, course) {
    ...
    if (currentrow.length > 0) {
        for (let i = 0; i < currentrow.length; i++) {
            ...
            let valuesRow = $(currentrow[i]).val().split('_/');
            let weekday = valuesRow[4];
            if (Number(valuesRow[2]) === Number(sem) && valuesRow[7] === sy && (Number(valuesRow[8]) === Number(room) || valuesRow[1] === course)){
                if (weekday === 'Monday'){
                    occupiedSched['start_time'] = Number(valuesRow[5]);
                    occupiedSched['end_time'] = Number(valuesRow[6]);
                    mondayArr.push(occupiedSched);
    ...
    let roomname = `#room option[value="${room}"]`;
    let teachername = `#teachers option[value="${teacher}"]`;
    $('#timeVacancyTitle').html(`Time Vacancy for <strong style="color: red;">${$(roomname).html()}</strong>, <strong style="color: green;">${course}</strong> and <strong style="color: blue;">${$(teachername).html()}</strong> on <strong style="color: orange;">Trimester - ${sem}</strong> S.Y <strong style="color: violet;">${sy}</strong>`);
...
$(document).ready(function() {
    ...
    // roombuilding
    $('#roombuilding').change(function() {
        // alert($(this).val());
        let rooms = '';
        $.getJSON(`../queries/getroom.php?getrooms=${$(this).val()}`, function(data) {
            data.forEach(room => {
                rooms += `<option value="${room.room_id}">${room.room_number} ${room.room_floor === '1' ?'1st ':room.room_floor === '2' ? '2nd ': room.room_floor === '3' ? '3rd ':'4th '}Floor</option>`;
            });
            $('#room').html(rooms);
...
function tapsched2(stringid) {
    ...
    if ($('#course').val() < 1 || subject < 1 || sem < 1 || sy < 1 || room < 1) {
    ...
    } else if (klasehanan.length > 0 && subject.length > 0 && sem > 0 && sy.length > 0 && room.length > 0) {
        if (start === end || start > end) {
            Swal.fire({
                icon: 'error',
                title: 'Oops...',
                text: 'Time cannot be the same and start time cannot be less than time end.',
                showConfirmButton: false,
                timer: 1000
            });
            $(stringid).prop('checked', false);
            ...
            $.getJSON(`../queries/getunit.php?subject=${subject}`,function(data){
                // alert();
                // unit = Number(data[0]);
                timecap = Number((((Number(data[0]) * 4)/3)*100).toFixed(2));
            });
            // mga values
            let queue = `#${days}`;
...
// kini na mga function kay pang detect sa subject kung mulapas ba siya ug 4 hours mag prompt na dayun kung ipadayun ba niyag check ang checkbox or dili
        ...
        for (let i = 0; i < data.length; i++) {
            if (data[i].length === 0){
                ...
                if (checkconflicts(i, data, start, end, subject) === 0) {
                    ...
                    return;
                } else if (checkconflicts(i, data, start, end, subject) === 2){
                    checkfalse(stringid);
                    Swal.fire({
                        icon: 'error',
...
function checkconflicts(counts, data, start, end, subject){
    for (let i = 0; i < data[counts].length; i++) {
        ...
        if (starttimedb === start && endtimedb === end) {
            if (counts === 2){
                if (subject === data[counts][i].subject){
                    return 'samesubject';
                } else {
                    return counts;
                }
