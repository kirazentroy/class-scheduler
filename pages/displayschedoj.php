...
<body>
                        ...
                        <div class="col-6">
                            <div class="row-mb-5">
                                ...
                                <div class="col-6 mb-3" id="parasasem">
                                    <label class="form-label fw-bold" for="semester">Semester</label>
                            ...
                            <div class="row" id="selectionby">
                                <div class="col-6 mb-2">
                                    ...
                                    <select class="form-select" name="selectby" id="selectby" onchange="choosenfilter();">
                                        ...
                                        <option value="courses">Strands/Grade</option>
                                    ...
                                    <div class="col-6">
                                        ...
                                        <select class="form-select" name="teachers" id="teachers" onchange="teacherEditDone();">
                                            <?php if ($_SESSION['permission'] === '1') {
                                                $teachers = executeNonQuery($connect, "SELECT * FROM members where member_activity = 'customized' and (member_superiority = 'admin' or member_superiority = 'faculty') order by member_department, members.member_salut, members.member_last");
                                            } else {
                                                $teachers = executeNonQuery($connect, "SELECT * FROM members where member_activity = 'customized' and (member_superiority = 'admin' or member_superiority = 'faculty') and member_id != '$id' order by member_department, members.member_salut, members.member_last");
                                            }
                                            $varSelf = executeNonQuery($connect, "SELECT * FROM members WHERE member_id = '$id'");
                                            $self = fetchAssoc($connect, $varSelf);
                                            ?>
                                            <option value="<?php echo $row['member_id']; ?>"><?php echo $row['member_salut'] . ' ' . $row['member_last'] . ', ' . $row['member_first']; ?></option>
                                            <?php
                                ...
                                <div class="row mt-2 d-none" id="parasacourse">
                                    <div class="col-6">
                                        <label class="form-label fw-bold" for="courses">Choose a Strand:</label><br>
                                        <select class="form-select" name="courses" id="courses" onchange="courseEditDone();">
                                            <?php
                                            $result = executeNonQuery($connect, "SELECT * FROM `scheduled_classes_shs` group by course order by course");
                                            $arrayofcourse = [];
    ...
    <script>
        ...
        function getFusedSchedules(row, idschedule, schedtype) {
            $.getJSON(`../queries/getfused.php?checkgetfusedshs=${idschedule}`, function(data) {
                if (data.length > 0) {
                    Swal.fire({
        ...
        function showeditschedmodal(row, idschedule, boolean, schedtype) {
            ...
            $('#tableid').val(row);
            $.getJSON(`../queries/editsched.php?getavailableshs=${idschedule}&schedtype=${schedtype}`, function(data) {
                if (data[0].length === 0) {
        ...
        function teacherEditDone() {
            ...
            let semfixed;
            if (semester === '1') {
                semfixed = '1st Semester';
            } else if (semester === '2') {
                semfixed = '2nd Semester';
            } else if (semester === '3') {
                semfixed = '3rd Semester';
            }
            let time = [
                [],
                {}
            ...
            $.getJSON(`../queries/getsched.php?getteachershs=${teacherid}&schoolyear=${schoolyear}&semester=${semester}`, function(data) {
        ...
        function courseEditDone() {
            ...
            $.getJSON(`../queries/getsched.php?getcourseshs=${coursename}&schoolyear=${schoolyear}&semester=${semester}`, function(data) {
        ...
        function showOrganizedTab() {
            window.open(`./pages/organizedschedule2.php?program=${$('#courses').val()}&semester=${$('#semester').val()}&sy=${$('#schoolyear').val()}`);
        }
        ...
        function roomEditDone() {
            ...
            $.getJSON(`../queries/getsched.php?getroomshs=${room}&schoolyear=${schoolyear}&semester=${semester}`, function(data) {
            ...
            for (let i = 0; i < arrayobj[0].length; i++) {
                ...
                for (let j = 0; j < arrayobj[1][arrayobj[0][i]].length; j++) {
                    ...
                    wew =
                        ...
                        <span>${arrayobj[1][arrayobj[0][i]][j].subject}</span><br>
