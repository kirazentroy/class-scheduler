...
<body>
                        ...
                        <tbody>
                            ...
                            ?>
                                    <tr id="tbodyrow_<?php echo $i ?>" <?php echo $rows['conflict_status'] === 'conflicted' ? 'style="background:red;"' : '' ?>>
                                        <td id="conflictchecking_<?php echo $i ?>" class="d-none"><?php echo $rows['start_time'] . '_/' . $rows['end_time'] . '_/' . $rows['teacher'] . '_/' . $rows['weekday'] . '_/' . $rows['room_id'] . '_/' . $rows['course'] . '_/' . $rows['schedsem'] . '_/' . $rows['schoolyear'] ?></td>
    ...
    <script>
        ...
        function approve(schedid, rowid) {
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    ...
                    let [st, nd, maestra, certainday, room, klasehanan, sem, sy] = $(checking).html().split('_/');
                        ...
                        $.getJSON(`../queries/getconflicts.php?getconflictsaprs=${sy}&semester=${sem}&weekday=${certainday}&room=${room}&schedule_process=approved`, function(data) {
                            if (data.length === 0) { // kung walay sulod then loop napud nga dili kapareho ug room
                                ...
                                $.getJSON(`../queries/getconflicts.php?getconflictsapr=${sy}&semester=${sem}&weekday=${certainday}&schedule_process=approved`, function(data2) {
                                    if (data2.length === 0) {
                                        ...
                                        for (let i = 0; i < data2.length; i++) {
                                            let starttimedb = Number(data2[i].start_time);
                                            let endtimedb = Number(data2[i].end_time);
                                            if (starttimedb >= 11200 && starttimedb <= 11259) {
                                                starttimedb -= 11200;
                                            }
                                            if (start >= 11200 && start <= 11259) {
                                                start -= 11200;
                                            }
                                            if (data2[i].teacher === maestra) {
                                                if ((data2[i].course === klasehanan) || data2[i].course != klasehanan) {
                                                    if (data2[i].start_time === start && endtimedb === end) {
                                                        document.location.href = `../functions/schedsprocessing.php?conflicted=${schedid}`;
                                                        return;
                                                    } else if ((start < data2[i].start_time && data2[i].start_time < end && end < endtimedb) || (data2[i].start_time < start && start < endtimedb && endtimedb < end)) {
                                                        document.location.href = `../functions/schedsprocessing.php?conflicted=${schedid}`;
                                                        return;
                                                    } else if ((start <= data2[i].start_time && start < endtimedb && end >= endtimedb && end > data2[i].start_time) || (data2[i].start_time <= start && data2[i].start_time < end && endtimedb >= end && endtimedb > start)) {
                                                        document.location.href = `../functions/schedsprocessing.php?conflicted=${schedid}`;
                                                        return;
                                                    } else if ((end <= data2[i].start_time && end < endtimedb && data2[i].start_time < endtimedb) || (endtimedb <= start && endtimedb < end && start < end)) {
                                                        continue;
                                                    }
                                                }
                                            } else if (data2[i].teacher != maestra) {
                                ...
                                for (let i = 0; i < data.length; i++) {
                                    let starttimedb = Number(data[i].start_time);
                                    let endtimedb = Number(data[i].end_time);
                                    if (starttimedb >= 11200 && starttimedb <= 11259) {
                                        starttimedb -= 11200;
                                    }
                                    if (start >= 11200 && start <= 11259) {
                                    ...
                                    if (data[i].teacher === maestra) {
                                        if ((data[i].course === klasehanan) || data[i].course != klasehanan) {
                                            if (data[i].start_time === start && endtimedb === end) {
                                                document.location.href = `../functions/schedsprocessing.php?conflicted=${schedid}`;
                                                return;
                                            } else if ((start < data[i].start_time && data[i].start_time < end && end < endtimedb) || (data[i].start_time < start && start < endtimedb && endtimedb < end)) {
                                                document.location.href = `../functions/schedsprocessing.php?conflicted=${schedid}`;
                                                return;
                                            } else if ((start <= data[i].start_time && start < endtimedb && end >= endtimedb && end > data[i].start_time) || (data[i].start_time <= start && data[i].start_time < end && endtimedb >= end && endtimedb > start)) {
                                                document.location.href = `../functions/schedsprocessing.php?conflicted=${schedid}`;
                                                return;
                                            } else if ((end <= data[i].start_time && end < endtimedb && data[i].start_time < endtimedb) || (endtimedb <= start && endtimedb < end && start < end)) {

                                                continue;
                                            }
                                        }
                                    } else if (data[i].teacher != maestra) {
        ...
        //     echo "Swal.fire({icon:'success', title:'$msg', showConfirmButton: false, timer:1500});";
        //     unset($_SESSION['approved']);
        // }

        // if (isset($_SESSION['disapproved'])) {
        //     $msg = $_SESSION['disapproved'];
        //     echo "Swal.fire({icon:'success', title:'$msg', showConfirmButton: false, timer:1500});";
